...
public class CaretVersion implements VersionRange {

  private final ExactVersion version;

  public CaretVersion(String version) {
    this.version = VersionParser.parseExact(version);
  }
  ...
  public boolean satisfiedBy(ExactVersion version) {
    ...
    final VersionPart pos = this.version.leftMostNonZeroPosition();

    if (pos == null || pos == VersionPart.Patch) {
      return version.compareTo(this.version) == 0;
    }

    final ExactVersion lessThan = this.version.withIncrement(this.version.leftMostNonZeroPosition());
